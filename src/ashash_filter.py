...
def readrib(ribfile, spatialResolution=1, af=4, rtree=None, filter=None):
    ...
    for line in p1.stdout: 
        ...
        node = rtree.add(zPfx)
        node.data[zOrig] = {"path": path, "count": 0}
...
def readupdates(filename, rtree, spatialResolution=1, af=4, filter=None):
    ...
    for line in p1.stdout:
        ...
        if res[2] == "W":
            ...
            # Check if the origin AS is in the filter:
            if not filter is None:
                # Check if the origin AS is in the filter or if the prefixes has
                # been announced by filtered ASs
                try:
                    origAS = int(path[-1])
                    if not origAS in filter:
                        continue
...
if __name__ == "__main__":
    ...
    for ribfile in rib_files:
        rtree = readrib(ribfile, args.spatial, args.af, rtree, filter)
